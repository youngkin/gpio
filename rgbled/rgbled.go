...
func ledColorSet(redVal, greenVal, blueVal uint32) {
	// This doesn't work as expected because Pin 19 & 13 are essentially linked. This
	// is also true for pin 18 & 12, but since pin 12 isn't used here pin 18, green,
...
func ledInit() {
	ledPinRed.Mode(rpio.Pwm)
	ledPinRed.Freq(freq)
	ledPinRed.DutyCycle(0, cycle)

	ledPinGreen.Mode(rpio.Pwm)
	ledPinGreen.Freq(freq)
	ledPinGreen.DutyCycle(1, cycle)

	ledPinBlue.Mode(rpio.Pwm)
	ledPinBlue.Freq(freq)
	ledPinBlue.DutyCycle(1023, cycle)
}
